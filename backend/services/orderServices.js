...
async function createOrderFromCart(userId, orderData) {
  try {
    ...
    console.log('📝 Order created (pre-save):', order);
    ...
    for (const cartItem of cart.items) {
      ...
      console.log(`🔗 Linked CartItem ${cartItem._id} to order`);
      ...
      console.log(`📉 Reduced stock for product ${cartItem.product._id}`);
    }
    
    // Update order with cart items
    savedOrder.items = cart.items.map(item => item._id);
    await savedOrder.save();
    console.log('✅ Order updated with items');
